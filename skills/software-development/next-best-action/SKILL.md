...
Before running the checklist, verify the skill file you are about to evaluate actually exists on disk. A ticket that "delivered" a skill (e.g., #1067 closed with "next-best-action done") does not guarantee the file was written. If the target skill is missing:
1. Check `~/.hermes/skills/<category>/<name>/SKILL.md`
2. If absent, create it from the Claude source at `~/.claude/skills/<name>/SKILL.md` before proceeding
3. This is a one-time bootstrap — subsequent Q1-Q6 runs will have the file available
...
lccjs is the only consumer today. When porting, parameterize:
- `npm run close` / `npm run error:log` are lccjs commands.
- Doc references (`docs/research/…`, `claude_workflow.md`, `RULES.md`) point at lccjs files.
